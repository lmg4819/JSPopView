...
//        NSIndexPath *beIdx = [self.calculator indexPathForDate:self.beginDate];
//        NSIndexPath *endIdx = [self.calculator indexPathForDate:self.endDate];
//         if (beIdx.section == section && beIdx) {
//             numberOfRows -= beIdx.row/7;
//         }
//        if (endIdx.section == section && endIdx) {
//            numberOfRows = endIdx.row/7;
//        }
...
- (void)scrollToDate:(NSDate *)date animated:(BOOL)animated
{
    ...
    NSInteger indexPathRow = [self.calculator indexPathForDate:date atMonthPosition:FSCalendarMonthPositionCurrent].row;
    ...
    if (!self.floatingMode) {
        ...
        CGRect headerFrame = [_collectionViewLayout layoutAttributesForSupplementaryViewOfKind:UICollectionElementKindSectionHeader atIndexPath:[NSIndexPath indexPathForItem:indexPathRow inSection:scrollOffset]].frame;
        CGRect frame = [_collectionViewLayout layoutAttributesForItemAtIndexPath:indexPath].frame;
        CGFloat y = MAX(0, frame.origin.y-headerFrame.size.height);
        y = MIN(y, _collectionView.contentSize.height-_collectionView.fs_height);
        [_collectionView setContentOffset:CGPointMake(0, y) animated:animated];
...
- (void)extracted:(BOOL)animated date:(NSDate *)date {
    
    [self scrollToDate:[self.gregorian fs_firstDayOfWeek:date] animated:animated];
//    [self scrollToDate:[self.gregorian fs_firstDayOfWeek:date] animated:animated];
...
- (void)scrollToPageForDate:(NSDate *)date animated:(BOOL)animated
{
    ...
    if (!self.floatingMode) {
        ...
        [self extracted:animated date:date];
    }
}
